...
export default function FacadePageClient() {
  const t = useTranslations('facade');
  const locale = useLocale();
  const list = placements.facade as Placement[];
      ...
      <DuskHero>
        <div className="shell">
          <div className="max-w-4xl mx-auto text-center">
            <span className="font-ui font-medium text-[11px] tracking-[0.3em] uppercase text-white/65 inline-block mb-6">{locale === 'en' ? '§ Facade Advertising' : '§ Fassadenwerbung'}</span>
            <h1
        ...
        <div className="shell flex flex-col gap-16 md:gap-24">
          {list.map((p, i) => {
            ...
            return (
              <div
                key={p.id}
                className={`grid lg:grid-cols-[1.1fr_1fr] grid-cols-1 gap-10 md:gap-14 items-center ${reverse ? 'lg:[grid-template-columns:1fr_1.1fr]' : ''}`}
              >
                {/* Image */}
                ...
                <div className={reverse ? 'lg:order-1' : ''}>
                  ...
                  <div className="grid grid-cols-3 border-t border-rule mb-6">
                    {[
                      ...
                      { v: p.duration, k: t('details.duration') ?? (locale === 'en' ? 'Period' : 'Periode') },
                    ].map((s, j) => (
                      <div key={j} className={`py-4 border-b border-rule ${j < 2 ? 'pr-3 border-r' : ''} ${j > 0 ? 'pl-3' : ''}`}>
                        <div className="font-display font-normal text-ink" style={{ fontSize: 'clamp(20px, 2.2vw, 28px)', lineHeight: 1, letterSpacing: '-0.02em' }}>
                          {s.v}
                        </div>
